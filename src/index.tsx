            ...
            <DataTable<IApplicationInfo>
                ...
                rows={{
                    ...
                    uxUserDocumentationRating: {
                        description: createTooltipDiv("User documentation rating", `
                    ...
                    uxSearchHighlights: {
                        description: createTooltipDiv("Results highlighted by query input", "Sometimes it can be difficult to know how a search matched the query you entered. Search highlights help this usability."),
                        css: colorBoolean,
                        Comp: ({ data }) => <div>{getPlannedBooleanItemIcon(data)}</div>,
                    },
                    uxLanguagesSupported: {
                        description: "Languages supported",
                        Comp: ({ data }) => <div>{data.map(e => Languages[e].symbol).join(" ")}</div>,
                    },
                    uxHasUndoRedo: {
                    ...
                    pluginsDocumentationRating: {
                        description: createTooltipDiv("Plugin/Developer Documentation Rating", `
                            1. Documentation doesn't exist
                            2. Documentation exists but is either difficult to find or covers very few features.
                            3. Documentation exists, is easy to find, adequate feature coverage.
                            4. Documentation exists, is easy to find, adequate feature coverage, with good use of key-display, diagrams, screenshots and videos
                    ...
                    hasTextInsertion: {
                        description: createTooltipDiv("Does the application have text insertion / word spreading?", "Does the application have the ability to insert text into another application, e.g. Alfred snippets."),
                        css: colorBoolean,
                        Comp: ({ data }) => <div>{getPlannedBooleanItemIcon(data)}</div>,
                    },
                    hasInstantSend: {
                        description: createTooltipDiv("Does the application have instant send", "This feature takes the currently selected item/text in another application and performs a search / action search based on the selected item."),
                        css: colorBoolean,
                        Comp: ({ data }) => <div>{getPlannedBooleanItemIcon(data)}</div>,
                    },
                    hasInAppMnemonics: {
                        description:
                            createTooltipDiv("Does the application have in-app mnemonics?", "Mnemonics overlay key combinations over UI elements in order to activate them with the keyboard."),
                        css: colorBoolean,
                        Comp: ({ data }) => <div>{getPlannedBooleanItemIcon(data)}</div>,
                    },
                    hasGlobalMnemonics: {
                        description:
                            createTooltipDiv("Does the application have global mnemonics?", "Global mnemonics overlay key combinations over other application's UI elements in order to activate them with the keyboard."),
                        css: colorBoolean,
                        Comp: ({ data }) => <div>{getPlannedBooleanItemIcon(data)}</div>,
                    },
                    hasSearchesOnActiveWindowContext: {
                        description: "Application obtains information about the previous focussed window to provide more context to search queries",
                        css: colorBoolean,
                        Comp: ({ data }) => <div>{getPlannedBooleanItemIcon(data)}</div>,
                    },
                    fileSearchExists: {
                    ...
                    webSearch: {
                        description: createTooltipDiv("Has WebSearch", "Web search allows launching of websites given a query input"),
                        css: colorBoolean,
                        Comp: ({ data }) => <div>{getPlannedBooleanItemIcon(data)}</div>,
                    },
                    webSearchIsCustomisable: {
                        description: createTooltipDiv("Is web search customisable?", "Customisable web search will allow users to query arbitrary websites"),
                        css: colorBoolean,
                        Comp: ({ data }) => <div>{getPlannedBooleanItemIcon(data)}</div>,
                    },
                    webBookmarkSearch: {
                    ...
                    settingsStyle: {
                        description: createTooltipDiv("How do you change settings?", `
